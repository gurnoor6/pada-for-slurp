...
def preproc(args):
   ...
   """
    ...
    for src in domains:
        ...
        train_path = "/home/gurnoor/massive/1.0/data/en-US.jsonl"
        with jsonlines.open(train_path) as f:
            for row in f:
                if row["partition"] != "train": continue
                dom_id = 1 if row["scenario"] == src else 0
                train.append(row["utt"])
                train_labels.append(dom_id)
        ...
        with jsonlines.open(train_path) as f:
            for row in f:
                ...
                dom_id = 1 if row["scenario"] == src else 0
                if dom_id == 0:
                    non_source.append(row["utt"])
                else:
                    source.append(row["utt"])
...
def main():
    ...
    parser.add_argument("--drf_set_location",
                        default='/home/gurnoor/PADA/slurp_adaptation/drf_sets',
                        type=str,
                        # required=True,
