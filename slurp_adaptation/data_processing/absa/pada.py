...
from slurp_adaptation.data_processing.absa.base import AbsaSeq2SeqDataProcessor
import torch
...
def test_AbsaSeq2SeqPadaDataset():
    ...
    l = len(data_processor.data[split]['output_labels_tokens'])

    data_dict = data_processor.data[split]
    for i in range(l):
        print(data_dict['input_str'][i])
        print(data_dict['output_labels_str'][i])
        print(data_dict['output_labels_multi'][i])
        print(data_dict['drfs'][i])
    # exit()
